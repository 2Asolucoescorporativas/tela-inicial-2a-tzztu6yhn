...
export function generatePwaReport(): string {
  ...
  lines.push('  Service worker precached /login (a SPA route, not a real file) and used it as')
  lines.push('  the navigation fallback. When the server returned 503 for /login, the precache')
  lines.push('  failed silently, leaving no cached fallback for SPA routes. All navigation')
  lines.push('  requests to /login returned 503 offline.')
  lines.push('')
  ...
  lines.push('  - sw.js: precaches /, /index.html, icons, and manifest (NOT /login)')
  lines.push('  - sw.js: navigation fallback returns cached / or /index.html (SPA shell)')
  lines.push('  - sw.js: activate clears ALL old caches by exact name match')
  ...
  lines.push('  - public/icons/ directory removed; no old icon files remain')
  lines.push('')
